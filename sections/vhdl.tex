\section{VHDL}
	Die vollständige Beschreibung des Designs besteht aus:\\
	\begin{itemize}
  ...
  \setlength{\parsep}{0pt}
	...
	\subsection{Bibliotheken}
		\begin{tabular}{ll}
			...
			std & Enthält standard (Vordefinierte Datentypen und Funktionen und\\
			& textio (Dateioperationen)\\
			ieee & std\_logic\_1164: Datentypen für mehrwertiges Logiksystem
		\end{tabular}
	...
	\subsection{Architekturbeschreibung}
		...
		\subsubsection{Komponentendeklaration}
			Mit Hilfe des Schlüsselwortes COMPONENT erfolgt die Deklaration von Komponenten für die möglicherweise mehrfache Instanziierung (Platzierung von Komponenten) in darüber liegenden hierarchischen Ebenen.\\
		...
		\subsubsection{Instanzierung}
			Hier werden die Blöcke (components) platziert und "`verdrahtet"'.
			\begin{multicols}{2}
...
\subsection{Prozesse}
	\begin{itemize}
  	...
  	\setlength{\parskip}{0pt}
  	\setlength{\parsep}{0pt}
		\item Prozesse werden durch Änderungen an den Signalen in der Sensitivitätsliste (im Prozess-Kopf) aktiviert und ausgeführt.
		...
		\item Gültig ist letzte Zuweisung
	\end{itemize}
